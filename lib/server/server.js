...
io.on('connection', (socket) => {
  ...
  socket.on(CLIENT_TYPING_EVENT, (status) => {
    socket.broadcast.to(room).emit(CLIENT_TYPING_EVENT, status);
  })

  io.in(room).emit(CLIENTS_CONNECTED_EVENT, numClients[room]);

  socket.on("disconnect", async () => {
    console.log("Client disconnected");
    numClients[socket.room]--;
    socket.broadcast.to(room).emit(CLIENT_TYPING_EVENT, false);
    socket.leave(room)
